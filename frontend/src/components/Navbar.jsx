...
import { Leaf, BarChart2, Menu, X, Calculator } from 'lucide-react';

const NAV_LINKS = [
  { to: '/',            label: 'Home',        page: 'home' },
  { to: '/go-green',    label: 'Go Green',    page: 'gogreen' },
  { to: '/the-reality', label: 'The Reality', page: 'reality' },
  { to: '/community',   label: 'Community',   page: 'community' },
...
export default function Navbar() {
  ...
  const isDark = ['/', '/the-reality', '/go-green'].includes(location.pathname) && !scrolled;
      ...
      >
        <div style={{ maxWidth: 1160, margin: '0 auto', padding: '0 40px', height: 72, display: 'flex', alignItems: 'center', gap: 32 }}>
          ...
          <div style={{ flexShrink: 0 }} className="desktop-nav">
            <Link to="/compare" className="btn btn--rust" style={{ display: 'inline-flex', alignItems: 'center', gap: 8 }}>
      ...
      <AnimatePresence>
        {menuOpen && (
          ...
          >
            ...
            <Link to="/compare" className="btn btn--rust" style={{ marginTop: 12, justifyContent: 'center' }}>
              Calculate Now
            </Link>
          </motion.div>
        )}
