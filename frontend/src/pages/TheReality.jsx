...
const STATS = [
  { num: '2.1M', label: 'Indians die from air pollution every year', src: 'WHO 2023' },
  { num: '14%', label: 'Of global CO₂ comes from transport', src: 'IEA 2023' },
  { num: '67%', label: "India's electricity still from coal", src: 'CEA 2023' },
  { num: '3.2M', label: 'New cars sold in India in 2023 — none showed lifecycle data', src: 'SIAM 2023' },
];
...
export default function TheReality() {
  return (
    <div style={{ paddingTop: 72 }}>
      {/* Hero */}
      <div style={{ background: 'var(--carbon)', color: 'var(--cream)', padding: '120px 0 80px' }}>
      ...
      <section style={{ background: 'var(--cream-dark)', padding: '80px 0' }}>
        <div className="container">
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 24 }}>
            {STATS.map((s, i) => (
              <motion.div key={i} initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} style={{ background: 'var(--white)', border: '1px solid var(--border)', borderRadius: 16, padding: 28 }}>
                <div style={{ fontFamily: 'var(--font-display)', fontSize: 42, fontWeight: 700, color: 'var(--rust)', lineHeight: 1, marginBottom: 10 }}>{s.num}</div>
                <div style={{ fontSize: 14, color: 'var(--text-secondary)', lineHeight: 1.5, marginBottom: 10 }}>{s.label}</div>
