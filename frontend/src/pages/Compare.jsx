...
import { ArrowLeft, BarChart2, Send, AlertTriangle, Trophy } from 'lucide-react';
...
export default function Compare() {
  ...
  async function askAI() {
    ...
    try {
      const res = await fetch('/api/chat/', {
        ...
        body: JSON.stringify({ message: q, context: `Comparing ${results.ra?.name} vs ${results.rb?.name}${results.rc ? ` vs ${results.rc?.name}` : ''} in ${country === 'india' ? state : country} at ${mileage}km/day for ${years} years. Totals: ${results.ra?.total}t, ${results.rb?.total}t${results.rc ? `, ${results.rc?.total}t` : ''}.` }),
      });
  ...
  function checkGW() {
    ...
    const FLAGS = [
      ...
      { term: 'sustainable', score: -5, msg: 'Unverified "sustainable" — no data' },
  ...
  const lineData = results ? {
    labels: Array.from({ length: 11 }, (_, i) => `${i * 10}k km`),
    datasets: cars.map(c => ({
      label: c.name,
      data: Array.from({ length: 11 }, (_, i) => {
        const km = i * 10000;
        const rate = c.fuel / (mileage * 365 * years);
        return +(c.mfg + c.battDisp + rate * km).toFixed(1);
      }),
      borderColor: c.type === 'ev' ? '#4A7C59' : c.type === 'hybrid' ? '#D4860A' : '#C44B2B',
          ...
          <motion.section ref={resultsRef} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} style={{ background: 'var(--cream-dark)', padding: '60px 0' }}>
            <div className="container">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28, marginBottom: 32 }}>
                ...
                <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 14, padding: 28 }}>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '.15em', textTransform: 'uppercase', color: 'var(--text-muted)', marginBottom: 20 }}>Cumulative CO₂ over lifetime (break-even)</div>
                  <Line data={lineData} options={{ ...chartOpts, plugins: { legend: { position: 'bottom', labels: { boxWidth: 12, font: { family: 'Space Mono', size: 10 } } } } }} />
                </div>
              </div>
                      ...
                      {cars.map(c => {
                        ...
                        return (
                          <td key={c.key} style={{ padding: '18px 20px', fontSize: row.bold ? 18 : 14, fontWeight: row.bold ? 700 : 400, color }}>
                            {val ? `${val}t CO₂` : '—'} {row.bold && val === minVal ? '✅' : row.bold ? '❌' : ''}
                          </td>
                        );
                      })}
                    </tr>
              ...
              <div style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: 14, padding: 32, marginTop: 28 }}>
                ...
                {gwResult && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ marginTop: 16, padding: 16, background: 'rgba(196,75,43,.06)', border: '1px solid rgba(196,75,43,.2)', borderRadius: 10 }}>
                    ...
                    {gwResult.flags.map((f, i) => <div key={i} style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 4 }}>🚩 {f}</div>)}
                  </motion.div>
                )}
              </div>
              ...
              {winner && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} style={{ background: 'linear-gradient(135deg, var(--olive) 0%, var(--olive-light) 100%)', color: 'var(--cream)', borderRadius: 14, padding: 36, marginTop: 32, display: 'flex', alignItems: 'center', gap: 32, flexWrap: 'wrap' }}>
                  ...
                  <div style={{ marginLeft: 'auto', display: 'flex', gap: 12 }}>
                    <button onClick={() => navigator.clipboard.writeText(window.location.href)} className="btn btn--ghost">
                      Share Results
                    </button>
