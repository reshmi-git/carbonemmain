...
const RANK_STYLES = [
  { cls: 'lb-rank--gold',   label: '01' },
  { cls: 'lb-rank--silver', label: '02' },
  { cls: 'lb-rank--bronze', label: '03' },
];

const GW_ALERTS = GREENWASH_DB.slice(0, 3);

const NEWS_CARDS = [
  { source: 'Down To Earth', title: "India's EV boom masks a coal problem", desc: 'India added 1.2M EVs in 2023 but 67% of its grid still runs on coal — making lifecycle comparisons critical.', url: 'https://www.downtoearth.org', tag: 'India' },
  { source: 'Carbon Brief',  title: 'Lifecycle analysis: EVs vs hybrids in coal-heavy grids', desc: 'New data shows hybrid advantage in high-coal grids. The coal context changes everything about EV calculations.', url: 'https://www.carbonbrief.org', tag: 'Research' },
  { source: 'Electrek',      title: 'Battery disposal: the ignored crisis in EV revolution', desc: 'As EV adoption accelerates, the industry faces a looming battery disposal challenge with real carbon implications.', url: 'https://electrek.co', tag: 'EV' },
];
...
export default function Home() {
  ...
  function checkGreenwash() {
    ...
    const flags = [];
    FLAGS.forEach(f => {
      if (lower.includes(f.term)) { score += f.score; if (f.msg) flags.push(f.msg); }
    });
  ...
  return (
    <div style={{ paddingTop: 72 }}>
      ...
      {/* ── HERO ── */}
      <section style={{ position: 'relative', minHeight: '100vh', background: 'var(--carbon)', color: 'var(--cream)', display: 'flex', alignItems: 'center', overflow: 'hidden', padding: '120px 0 80px' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 80% 60% at 70% 50%, rgba(61,74,46,0.18) 0%, transparent 65%), radial-gradient(ellipse 50% 80% at 20% 80%, rgba(196,75,43,0.06) 0%, transparent 60%), var(--carbon)' }} />
        {/* Grain */}
        ...
        <div style={{ maxWidth: 1160, margin: '0 auto', padding: '0 40px', display: 'grid', gridTemplateColumns: '1fr 380px', gap: 60, alignItems: 'center', position: 'relative', zIndex: 2, width: '100%' }}>
          <div>
            ...
            <motion.h1 {...fadeUp(0.2)} style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(52px,6vw,84px)', fontWeight: 700, lineHeight: 1.05, letterSpacing: '-.025em', color: 'var(--cream)', marginBottom: 24 }}>
              Your car has a<br /><em style={{ fontStyle: 'italic', color: 'var(--olive-muted)' }}>secret carbon</em><br />past.
            </motion.h1>
            <motion.p {...fadeUp(0.3)} style={{ fontSize: 17, color: 'rgba(245,240,232,0.65)', lineHeight: 1.7, maxWidth: 440, marginBottom: 36 }}>
              Manufacturing. Fuel. Battery disposal. We show the full lifecycle cost — adjusted for your grid, state, and how you actually drive.
            </motion.p>
          ...
          {/* Hero widget */}
          <motion.div initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.4, duration: 0.7 }} style={{ background: 'rgba(245,240,232,0.04)', border: '1px solid rgba(245,240,232,0.1)', borderRadius: 20, padding: 28, backdropFilter: 'blur(10px)' }} className="hero-widget">
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, letterSpacing: '.14em', textTransform: 'uppercase', color: 'rgba(245,240,232,0.3)', marginBottom: 20 }}>Live Leaderboard · Maharashtra · 40km/day · 8yr</div>
            {leaderboard.slice(0, 4).map((car, i) => (
              <div key={car.key} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 0', borderBottom: i < 3 ? '1px solid rgba(245,240,232,0.06)' : 'none' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 32, flexWrap: 'wrap' }}>
            ...
            <motion.div initial={{ opacity: 0, x: -28 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} style={{ flex: 1, minWidth: 280, background: 'var(--cream-dark)', border: '1.5px solid var(--border)', borderRadius: 20, padding: 36 }}>
              ...
              {[{ icon: <Fuel size={22} color="var(--olive)" />, label: 'Fuel efficiency', val: '18 km/L ✓', hidden: false },
                ...
                { icon: <Battery size={22} color="#ccc" />, label: 'Battery disposal', val: 'not shown', hidden: true }].map(m => (
            ...
            <motion.div initial={{ opacity: 0, x: 28 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} style={{ flex: 1, minWidth: 280, background: 'var(--carbon)', color: 'var(--cream)', border: '1.5px solid rgba(245,240,232,0.08)', borderRadius: 20, padding: 36 }}>
              ...
              {[{ icon: <Factory size={22} color="var(--rust)" />, label: '🏭 Manufacturing', val: '12t CO₂', color: 'var(--rust)' },
                ...
                { icon: <Battery size={22} color="var(--olive-muted)" />, label: '🔋 Battery Disposal', val: '8t CO₂', color: 'var(--olive-muted)' }].map(m => (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {filtered.map((car, i) => {
                ...
                return (
                  <motion.tr key={car.key} initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.06 }} style={{ borderBottom: '1px solid rgba(245,240,232,0.05)' }}>
        ...
        <div className="container">
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 420px', gap: 48, alignItems: 'start' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              {GW_ALERTS.map((gw, i) => (
                <motion.div key={i} initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} style={{ display: 'flex', gap: 16, padding: '20px 24px', background: 'var(--white)', border: '1.5px solid rgba(196,75,43,0.15)', borderRadius: 14, transition: 'all 0.2s' }}>
      ...
      <section style={{ background: 'var(--olive)', padding: '100px 0', color: 'var(--cream)' }}>
        <div className="container" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 80, alignItems: 'center' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 28 }}>
            {[['2.1M', 'Indians die from air pollution yearly'], ['14%', 'Global CO₂ from transport'], ['67%', "India's electricity from coal"], ['₹0', 'Carbon cost shown in any car ad']].map(([num, label]) => (
              <motion.div key={label} whileInView={{ opacity: 1, y: 0 }} initial={{ opacity: 0, y: 20 }} viewport={{ once: true }} style={{ background: 'rgba(245,240,232,0.06)', border: '1px solid rgba(245,240,232,0.1)', borderRadius: 14, padding: 24 }}>
                <div style={{ fontFamily: 'var(--font-display)', fontSize: 32, fontWeight: 700, color: 'var(--cream)', marginBottom: 4 }}>{num}</div>
                <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'rgba(245,240,232,0.5)', letterSpacing: '.08em' }}>{label}</div>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section style={{ background: 'var(--cream)', padding: '100px 0' }}>
        <div className="container">
          <div style={{ display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', marginBottom: 48, flexWrap: 'wrap', gap: 24 }}>
            ...
            <Link to="/community" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, fontFamily: 'var(--font-mono)', fontSize: 12, fontWeight: 700, color: 'var(--olive)', letterSpacing: '.08em' }}>
              See all <ArrowUpRight size={14} />
            </Link>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 24 }}>
            {NEWS_CARDS.map((card, i) => (
              <motion.a key={i} href={card.url} target="_blank" rel="noopener noreferrer" initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1 }} whileHover={{ y: -6, boxShadow: '0 12px 48px rgba(0,0,0,0.13)' }} style={{ background: 'var(--white)', border: '1px solid var(--border)', borderRadius: 20, overflow: 'hidden', display: 'flex', flexDirection: 'column', textDecoration: 'none', color: 'inherit' }}>
                <div style={{ height: 180, background: 'var(--cream-dark)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexDirection: 'column', gap: 10, color: 'var(--text-muted)', opacity: 0.4 }}>
                  <BarChart2 size={32} />
                </div>
                <div style={{ padding: 24, flex: 1, display: 'flex', flexDirection: 'column' }}>
                  <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '.1em', textTransform: 'uppercase', color: 'var(--olive)', marginBottom: 10, fontWeight: 700 }}>{card.source}</div>
                  <div style={{ fontFamily: 'var(--font-display)', fontSize: 18, fontWeight: 700, lineHeight: 1.3, marginBottom: 10 }}>{card.title}</div>
                  <div style={{ fontSize: 13, color: 'var(--text-muted)', lineHeight: 1.7, flex: 1 }}>{card.desc}</div>
